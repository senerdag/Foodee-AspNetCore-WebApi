...
namespace Foodee.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminContactInfoController : Controller
    {
        ...
        public async Task<IActionResult> CreateContactInfo(CreateContactInfoDto createContactInfoDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7122/api/ContactInfos", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7122/api/ContactInfos?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminContactInfo", new { area = "Admin" });
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7122/api/ContactInfos/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateContactInfo(UpdateContactInfoDto updateContactInfoDto)
        {
            ...
            var responseMessage = await client.PutAsync("https://localhost:7122/api/ContactInfos/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminContactInfo", new { area = "Admin" });
            }
