...
namespace Foodee.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminBannerController : Controller
    {
        ...
        public async Task<IActionResult> CreateBanner(CreateBannerDto createBannerDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7122/api/Banners", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7122/api/Banners?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminBanner", new { area = "Admin" });
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7122/api/Banners/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateBanner(UpdateBannerDto updateBannerDto)
        {
            ...
            var responseMessage = await client.PutAsync("https://localhost:7122/api/Banners/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminBanner", new { area = "Admin" });
            }
